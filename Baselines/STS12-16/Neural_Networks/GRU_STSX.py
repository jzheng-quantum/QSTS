...
class GRUEncoder(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(gru_hidden_dim, reduced_dim).double()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        batch_range = torch.arange(hidden_seq.size(0),
                                   device=hidden_seq.device)
        last_hidden = hidden_seq[batch_range, last_idx, :]
    ...
    def count_parameters(self) -> Dict[str, int]:
        ...
        gru = self._numel(p for p in self.gru_parameters() if p.requires_grad)
        return {
            "embedding": 0,
            "linear": lin,
            "gru": gru,
            "total_trainable": lin + gru,
        }


class SentenceSimilarityGRU(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        embedding_dim: int,
...
def run_single_seed(
    ...
    *,
    ...
    header = (
        ...
        f"  - GRU             : GRU(input={REDUCED_DIM}, hidden={GRU_HIDDEN_DIM}, "
        f"layers={GRU_NUM_LAYERS}, batch_first) + last-hidden + Linear({GRU_HIDDEN_DIM}->{REDUCED_DIM})\n"
...
def main() -> None:
    ...
    variant_tag = (
        f"gru{GRU_HIDDEN_DIM}_layers{GRU_NUM_LAYERS}_LOO"
    )
