...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="BoW (parameter-free) baseline on STS-12..STS-16 "
...
def main() -> None:
    ...
    train_only_corpus: List[str] = []
    for item in train_records_for_vocab:
        train_only_corpus.append(str(item["sentence1"]))
        train_only_corpus.append(str(item["sentence2"]))
    vectorizer.fit(train_only_corpus)
    ...
    vocab_summary = (
        ...
        f"OOV (silently dropped at transform time)\n"
