...
def main() -> None:
    ...
    model_name = "Hash"
    dataset_name = "STSB"
    variant_tag = f"hash_nfeat{args.n_features}"
    ...
    header = (
        ...
        f"  - features     : n_features={args.n_features}, alternate_sign=True, L2-normalised\n"
    ...
    train_collision_ratio = len(train_token_set) / float(args.n_features)
    union_collision_ratio = len(union_token_set) / float(args.n_features)

    vocab_summary = (
        f"Unique tokens in training set:             {len(train_token_set)}\n"
        f"Hash buckets (n_features):                 {args.n_features}\n"
        f"Tokens-per-bucket (training pressure):     {train_collision_ratio:.4f}\n"
        f"Total word types across train, validation, and test: {len(union_token_set)}\n"
        f"Tokens-per-bucket (union pressure):        {union_collision_ratio:.4f}\n"
        f"  - Validation set: {len(val_token_set)} word types; "
        f"{len(val_oov)} ({100.0 * len(val_oov) / max(1, len(val_token_set)):.2f}%) "
        f"are not seen in training (still hashed: stateless vectorizer)\n"
