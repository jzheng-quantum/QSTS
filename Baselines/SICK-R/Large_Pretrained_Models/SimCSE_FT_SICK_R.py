...
class SimCSEFineTuneEncoder(nn.Module):
    ...
    def __init__(self, simcse_model_path: str, reduced_dim: int,
        ...
        self.linear = nn.Linear(SIMCSE_HIDDEN, reduced_dim)
    ...
    def forward(self,
        ...
        if self.simcse_variant == "sup":
            ...
            cls_vec = simcse_out.pooler_output
        elif self.simcse_variant == "unsup":

            cls_vec = simcse_out.last_hidden_state[:, 0, :]
        else:

            raise RuntimeError(
                f"Unknown simcse_variant: {self.simcse_variant}"
            )
        sent_vec = self.linear(cls_vec)
        return sent_vec
...
class SentenceSimilaritySimCSE(nn.Module):
    ...
    def __init__(self, simcse_model_path: str, reduced_dim: int,
                 simcse_variant: str):
        super().__init__()
        self.encoder = SimCSEFineTuneEncoder(simcse_model_path, reduced_dim,
...
def build_param_groups(model: SentenceSimilaritySimCSE) -> List[Dict]:
    
    no_decay_keywords = ("bias", "LayerNorm.weight", "LayerNorm.bias")

    simcse_decay_params: List[nn.Parameter] = []
    simcse_no_decay_params: List[nn.Parameter] = []
    ...
    for name, param in model.named_parameters():
        ...
        if name.startswith("encoder.simcse."):
            if is_no_decay:
                simcse_no_decay_params.append(param)
            else:
                simcse_decay_params.append(param)
    ...
    if simcse_decay_params:
        param_groups.append({
            "params": simcse_decay_params,
            "lr": SIMCSE_LR, "weight_decay": SIMCSE_WD,
            "name": "simcse_decay",
        })
    if simcse_no_decay_params:
        param_groups.append({
            "params": simcse_no_decay_params,
            "lr": SIMCSE_LR, "weight_decay": 0.0,
            "name": "simcse_no_decay",
        })
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--simcse_variant", type=str,
                        ...
                        help="Which official SimCSE checkpoint to fine-tune. "
...
def run_single_seed(seed: int,
    ...
    header = (
        ...
        f"fully fine-tuned (all params trainable)\n"
        ...
        f"  - SimCSE lr       : {SIMCSE_LR}  (weight_decay={SIMCSE_WD}, "
        ...
        f"  - precision       : fp32 (SimCSE native)\n"
...
def main() -> None:
    ...
    variant_tag    = f"finetune_{pooler_short_tag}"
