...
def e5_model_dir(base_dir: str) -> str:
    
    return str(Path(base_dir) / E5_DIRNAME)
...
class SentencePairDataset(Dataset):
    ...
    def __init__(self, data: pd.DataFrame, tokenizer, max_len: int):
        ...
        for _, row in data.iterrows():
            ...
            sent_a = E5_PREFIX + str(row["sentence_A"])
            sent_b = E5_PREFIX + str(row["sentence_B"])
...
def evaluate_split(model: BertModel,
    ...
    for (input_ids_a, attention_mask_a,
        ...
        v_a = F.normalize(v_a, p=2, dim=1, eps=EPS)
        v_b = F.normalize(v_b, p=2, dim=1, eps=EPS)
        cos = (v_a * v_b).sum(dim=1)
        sim = (cos + 1.0) / 2.0
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model_base_dir", type=str,
                        default="./",
                        help="Directory that contains the e5-base-v2 model "
                             "directory. The resolved model path is "
                             f"<model_base_dir>/{E5_DIRNAME}.")
    parser.add_argument("--pooling",    type=str,
                        default=DEFAULT_POOLING,
                        choices=list(POOLING_CHOICES),
                        help="Sentence-pooling strategy on top of the frozen "
                             "E5 encoder. 'mean' is E5's official protocol "
                             "(attention-mask-weighted mean over tokens). "
                             "'cls' is provided for completeness but is not "
                             "recommended (E5 was trained with mean-pool). "
...
def main() -> None:
    ...
    model_path = e5_model_dir(args.model_base_dir)
    if not Path(model_path).is_dir():
        raise FileNotFoundError(
            f"E5 model directory not found: {model_path}.  "
            f"Expected layout: '{args.model_base_dir}/{E5_DIRNAME}'."
        )


    if args.pooling == "mean":
        pooling_desc = ("attention-mask-weighted mean over token positions "
                        "(E5's official protocol, Wang et al. 2022)")
    elif args.pooling == "cls":
        pooling_desc = ("last_hidden_state[:, 0, :]  "
                        "(first token; NOT recommended for E5, provided for "
                        "completeness only)")
    else:
        pooling_desc = f"<unknown pooling: {args.pooling}>"

    header = (
        f"=== Zero-shot E5 evaluation on SICK-R ===\n"
        f"run_stamp={run_stamp}\n"
        f"model={model_name}\n"
        f"  - checkpoint      : intfloat/e5-base-v2 "
        f"(Wang et al., Microsoft 2022)\n"
        f"  - model path      : {model_path}\n"
        f"  - architecture    : BertModel (BERT-base backbone, hidden_size={E5_HIDDEN})\n"
        f"  - tokenizer       : AutoTokenizer (BertTokenizerFast / WordPiece), "
        f"max_len={args.max_len}\n"
        f"  - input prefix    : {E5_PREFIX!r}  "
        f"(E5 convention for symmetric STS tasks)\n"
        f"  - pooling         : {args.pooling}\n"
        f"  - sentence vector : {pooling_desc}\n"
        f"  - post-processing : L2-normalize each sentence vector "
        f"(E5 official step)\n"
        f"  - similarity      : (cos(v1, v2) + 1) / 2   in [0, 1], "
        f"computed as dot-product of unit vectors\n"
    ...
    pc_lines = [
        "Parameter breakdown:",
        f"  - E5 (frozen, non-trainable) : {total_params:,}",
        f"  - Trainable                  : 0",
    ]
