...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pooling",    type=str, default=DEFAULT_POOLING,
                        ...
                        help="Sentence-pooling strategy on top of fine-tuned "
                             "RoBERTa. 'cls' = last_hidden_state[:, 0, :] "
                             "(<s> token, BERT-style classification recipe). "
                             "'mean' = mean over non-padding tokens with "
                             "attention_mask (Sentence-BERT-style, Reimers & "
                             f"Gurevych 2019). Default: '{DEFAULT_POOLING}'.")
    return parser
...
def run_single_seed(seed: int,
    ...
    if args.pooling == "cls":
        pooling_desc = ("last_hidden_state[:, 0, :]  (i.e. <s> token in "
                        "RoBERTa, BERT-style classification recipe)")
