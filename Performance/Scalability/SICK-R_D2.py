...
def qsts_circuit(params, wires=None):
    """D2 ablation: depth = 2 sandwich blocks per sentence (block params independent).
    
    Layout (144 params total):
      sentence A (wires[0..5], 72 params):
        block 1 : params[0..35]   (S · CB^5 · S')
        block 2 : params[36..71]  (S · CB^5 · S')
      sentence B (wires[6..11], 72 params):
        block 1 : params[72..107]
        block 2 : params[108..143]
    ...
    _sandwich_block(params[72:108],  wires=sentence_b_wires)
...
class QuantumNeuralNetwork(nn.Module):
    def __init__(
        ...
        self.params = nn.Parameter(
            torch.rand(144, generator=quantum_rng, dtype=torch.float64) * 2 * math.pi
        )
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--log_dir", type=str, default="./logs_D2_TrainVocab")
...
def main() -> None:
    ...
    dataset_name = "SICK"
    variant_tag = "D2_sandwich_CB5_depth2blocks_independentparams_frozen_trainvocab"
    run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    log_dir = Path(args.log_dir)
    log_dir.mkdir(parents=True, exist_ok=True)
    base_name = (
        f"QSTS-D2_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ablation        : DEPTH (L=2 sandwich blocks per sentence; total wires = 6+6+1 = 13)\n"
        ...
        f"  - ansatz          : 2 stacked sandwich blocks  [S(θ) · C_{topology_name}^5 · S(θ')] × 2  (block params INDEPENDENT)\n"
        f"  - entangling      : 5 layers of {topology_name} CNOTs per block × 2 blocks = 60 CNOTs/sentence\n"
        f"  - quantum params  : 144 (Sentence A: 72 = 2×36; Sentence B: 72 = 2×36; independent A/B and inter-block)\n"
