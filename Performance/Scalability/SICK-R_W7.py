...
def qsts_circuit(params, wires=None):
    """W7 ablation: each sentence uses 7 qubits, CB entangler repeated x6 (n-1 = 6).
    
    Layout (84 params total):
      sentence A (wires[0..6], 42 params):
        front S layer  : params[0..20]   (7 single_u, 21 params)
        CB(7q)^6       : 6 layers of 7 CNOTs = 42 CNOTs, no params
        back  S layer  : params[21..41]  (7 single_u, 21 params)
      sentence B (wires[7..13], 42 params):
        front S layer  : params[42..62]
        CB(7q)^6       : 6 layers of 7 CNOTs
        back  S layer  : params[63..83]
    """
    # ---------- Sentence A on wires[0..6] ----------
    # Front S layer (21 params)
    ...
    single_u(params[18:21], wires=wires[6])

    # CB(7q)^6 entangling: 6 layers, each layer = 7 CNOTs (descending ring)
    for _ in range(6):
        qml.CNOT(wires=[wires[6], wires[0]])
    ...
    # Back S layer (21 params)
    single_u(params[21:24], wires=wires[0])
    single_u(params[24:27], wires=wires[1])
    single_u(params[27:30], wires=wires[2])
    single_u(params[30:33], wires=wires[3])
    single_u(params[33:36], wires=wires[4])
    single_u(params[36:39], wires=wires[5])
    single_u(params[39:42], wires=wires[6])

    # ---------- Sentence B on wires[7..13] ----------
    # Front S layer (21 params)
    single_u(params[42:45], wires=wires[7])
    single_u(params[45:48], wires=wires[8])
    single_u(params[48:51], wires=wires[9])
    single_u(params[51:54], wires=wires[10])
    single_u(params[54:57], wires=wires[11])
    single_u(params[57:60], wires=wires[12])
    single_u(params[60:63], wires=wires[13])

    # CB(7q)^6 entangling
    for _ in range(6):
        qml.CNOT(wires=[wires[13], wires[7]])
        qml.CNOT(wires=[wires[12], wires[13]])
        qml.CNOT(wires=[wires[11], wires[12]])
    ...
    # Back S layer (21 params)
    single_u(params[63:66], wires=wires[7])
    single_u(params[66:69], wires=wires[8])
    single_u(params[69:72], wires=wires[9])
    single_u(params[72:75], wires=wires[10])
    single_u(params[75:78], wires=wires[11])
    single_u(params[78:81], wires=wires[12])
    single_u(params[81:84], wires=wires[13])
...
class QuantumNeuralNetwork(nn.Module):
    def __init__(
        ...
        self.params = nn.Parameter(
            torch.rand(84, generator=quantum_rng, dtype=torch.float64) * 2 * math.pi
        )
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--log_dir", type=str, default="./logs_W7_TrainVocab")
...
def main() -> None:
    ...
    dataset_name = "SICK"
    variant_tag = "W7_sandwich_CB6_7qubitsPerSentence_independentparams_frozen_trainvocab"
    run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    log_dir = Path(args.log_dir)
    log_dir.mkdir(parents=True, exist_ok=True)
    base_name = (
        f"QSTS-W7_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ablation        : WIDTH (n=7 qubits per sentence; total wires = 7+7+1 = 15)\n"
        f"  - reduced dim     : 128 (= 2^7, AmplitudeEmbedding)\n"
        f"  - embedding layer : FROZEN (requires_grad=False)\n"
        f"  - vocabulary      : TRAIN ONLY (inductive; OOV in val/test → <UNK>)\n"
        f"  - ansatz          : sandwich  U(θ₁,θ₂) = S(θ₂) · C_{topology_name}^6 · S(θ₁)  (params INDEPENDENT front/back)\n"
        f"  - entangling      : 6 layers of {topology_name} CNOTs per block (n-1 = 6; 7 CNOTs/layer; 42 CNOTs/block)\n"
        f"  - quantum params  : 84 (Block A: 21+21 for front/back S layers; Block B: 21+21; independent A/B)\n"
