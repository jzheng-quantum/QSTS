...
def _sandwich_block(params, wires):
    """One full sandwich block on 6 qubits: S(θ_front) · CB^5 · S(θ_back).
    
    Uses 36 params: 18 for front S layer, 18 for back S layer.
    Entangling part is 5 layers of CB CNOTs (parameter-free), 30 CNOTs total.
    
    Args:
        params : 1D array of length 36
        wires  : list of 6 wires
    """
    # Front S layer: 6 single_u (18 params)
    ...
    # CB^5 entangling: 5 layers, each layer = 6 CNOTs (descending ring)
    for _ in range(5):
    ...
    # Back S layer: 6 single_u (18 params)
...
def qsts_circuit(params, wires=None):
    """D3 ablation: depth = 3 sandwich blocks per sentence (block params independent).
    
    Layout (216 params total):
      sentence A (wires[0..5], 108 params):
        block 1 : params[0..35]      (S · CB^5 · S')
        block 2 : params[36..71]     (S · CB^5 · S')
        block 3 : params[72..107]    (S · CB^5 · S')
      sentence B (wires[6..11], 108 params):
        block 1 : params[108..143]
        block 2 : params[144..179]
        block 3 : params[180..215]
    Blocks are stacked with no extra entangling layer between them
    (back-S' of block N flows directly into front-S of block N+1).
    """
    # ---------- Sentence A on wires[0..5] ----------
    sentence_a_wires = [wires[i] for i in range(6)]
    _sandwich_block(params[0:36],    wires=sentence_a_wires)
    _sandwich_block(params[36:72],   wires=sentence_a_wires)
    _sandwich_block(params[72:108],  wires=sentence_a_wires)

    # ---------- Sentence B on wires[6..11] ----------
    sentence_b_wires = [wires[i] for i in range(6, 12)]
    _sandwich_block(params[108:144], wires=sentence_b_wires)
    _sandwich_block(params[144:180], wires=sentence_b_wires)
    _sandwich_block(params[180:216], wires=sentence_b_wires)
...
class QuantumNeuralNetwork(nn.Module):
    def __init__(
        ...
        self.params = nn.Parameter(
            torch.rand(216, generator=quantum_rng, dtype=torch.float64) * 2 * math.pi
        )
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--log_dir", type=str, default="./logs_D3_TrainVocab")
...
def main() -> None:
    ...
    dataset_name = "SICK"
    variant_tag = "D3_sandwich_CB5_depth3blocks_independentparams_frozen_trainvocab"
    run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    log_dir = Path(args.log_dir)
    log_dir.mkdir(parents=True, exist_ok=True)
    base_name = (
        f"QSTS-D3_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ablation        : DEPTH (L=3 sandwich blocks per sentence; total wires = 6+6+1 = 13)\n"
        f"  - reduced dim     : 64 (= 2^6, AmplitudeEmbedding)\n"
        f"  - embedding layer : FROZEN (requires_grad=False)\n"
        f"  - vocabulary      : TRAIN ONLY (inductive; OOV in val/test → <UNK>)\n"
        f"  - ansatz          : 3 stacked sandwich blocks  [S(θ) · C_{topology_name}^5 · S(θ')] × 3  (block params INDEPENDENT)\n"
        f"  - entangling      : 5 layers of {topology_name} CNOTs per block × 3 blocks = 90 CNOTs/sentence\n"
        f"  - quantum params  : 216 (Sentence A: 108 = 3×36; Sentence B: 108 = 3×36; independent A/B and inter-block)\n"
