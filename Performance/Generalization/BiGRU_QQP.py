...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="BiGRU baseline on QQP (16-dim, hidden=64, layers=1, bidirectional).",
    )
    parser.add_argument(
...
def run_single_seed(
    ...
    *,
    ...
    header = (
        ...
        f"pack_padded_sequence + concat(h_n forward, h_n backward) + "
        f"Linear({2*GRU_HIDDEN_DIM}->{REDUCED_DIM})\n"
