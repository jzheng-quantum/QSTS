...
def _a2aref_block(params, wires):
    

    for q in range(6):
        qml.RX(params[q], wires=wires[q])
    for q in range(6):
        qml.RZ(params[6 + q], wires=wires[q])


    p = 12

    qml.CRX(params[p],     wires=[wires[5], wires[4]])
    qml.CRX(params[p + 1], wires=[wires[5], wires[3]])
    qml.CRX(params[p + 2], wires=[wires[5], wires[2]])
    qml.CRX(params[p + 3], wires=[wires[5], wires[1]])
    qml.CRX(params[p + 4], wires=[wires[5], wires[0]])
    p += 5

    qml.CRX(params[p],     wires=[wires[4], wires[5]])
    qml.CRX(params[p + 1], wires=[wires[4], wires[3]])
    qml.CRX(params[p + 2], wires=[wires[4], wires[2]])
    qml.CRX(params[p + 3], wires=[wires[4], wires[1]])
    qml.CRX(params[p + 4], wires=[wires[4], wires[0]])
    p += 5

    qml.CRX(params[p],     wires=[wires[3], wires[5]])
    qml.CRX(params[p + 1], wires=[wires[3], wires[4]])
    qml.CRX(params[p + 2], wires=[wires[3], wires[2]])
    qml.CRX(params[p + 3], wires=[wires[3], wires[1]])
    qml.CRX(params[p + 4], wires=[wires[3], wires[0]])
    p += 5

    qml.CRX(params[p],     wires=[wires[2], wires[5]])
    qml.CRX(params[p + 1], wires=[wires[2], wires[4]])
    qml.CRX(params[p + 2], wires=[wires[2], wires[3]])
    qml.CRX(params[p + 3], wires=[wires[2], wires[1]])
    qml.CRX(params[p + 4], wires=[wires[2], wires[0]])
    p += 5

    qml.CRX(params[p],     wires=[wires[1], wires[5]])
    qml.CRX(params[p + 1], wires=[wires[1], wires[4]])
    qml.CRX(params[p + 2], wires=[wires[1], wires[3]])
    qml.CRX(params[p + 3], wires=[wires[1], wires[2]])
    qml.CRX(params[p + 4], wires=[wires[1], wires[0]])
    p += 5

    qml.CRX(params[p],     wires=[wires[0], wires[1]])
    qml.CRX(params[p + 1], wires=[wires[0], wires[2]])
    qml.CRX(params[p + 2], wires=[wires[0], wires[3]])
    qml.CRX(params[p + 3], wires=[wires[0], wires[4]])
    qml.CRX(params[p + 4], wires=[wires[0], wires[5]])
    p += 5


    for q in range(6):
        qml.RX(params[42 + q], wires=wires[q])
    for q in range(6):
        qml.RZ(params[48 + q], wires=wires[q])


def qsts_circuit(params, wires=None):
    
    _a2aref_block(params[0:54],   wires=[wires[i] for i in range(6)])
    _a2aref_block(params[54:108], wires=[wires[i] for i in range(6, 12)])
...
class QuantumNeuralNetwork(nn.Module):
    def __init__(
        ...
        self.params = nn.Parameter(
            torch.rand(108, generator=quantum_rng, dtype=torch.float64) * 2 * math.pi
        )
...
def main() -> None:
    ...
    base_name = (
        f"A2Aref_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ansatz          : A2A-ref  (Reference All-to-All, RX+RZ single-qubit, CRX entanglers)\n"
        f"  - single-qubit    : RX(θ) RZ(θ) per qubit (12 params per single-qubit layer)\n"
        f"  - entangling      : 30 CRX gates (all ordered pairs ctrl→tgt, 6 ctrl × 5 tgt), NOT repeated\n"
        f"                       order: ctrl=5,4,3,2,1,0 (descending); each controls all other 5 qubits\n"
        f"  - quantum params  : 108 total (Block A: 12+30+12=54; Block B: 12+30+12=54; independent A/B)\n"
