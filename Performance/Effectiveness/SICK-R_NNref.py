...
def _nnref_block(params, wires):
    ...
    for _ in range(6):
        ...
        qml.CRX(params[p + 1], wires=[wires[2], wires[3]])
        qml.CRX(params[p + 2], wires=[wires[4], wires[5]])

        qml.CRX(params[p + 3], wires=[wires[1], wires[2]])
        qml.CRX(params[p + 4], wires=[wires[3], wires[4]])
        p += 5
...
def qsts_circuit(params, wires=None):
    ...
    _nnref_block(params[0:54],   wires=[wires[i] for i in range(6)])
    _nnref_block(params[54:108], wires=[wires[i] for i in range(6, 12)])
...
def main() -> None:
    ...
    base_name = (
        f"NNref_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ansatz          : NN-ref  (Reference Brick-wall, RX+RZ single-qubit, CRX entanglers)\n"
        f"  - single-qubit    : RX(θ) RZ(θ) per qubit (12 params per single-qubit layer)\n"
        f"  - entangling      : 5 CRX gates per unit, brick-wall pattern, repeated x6 (30 CRX/block)\n"
        f"                       per unit: CRX(0,1), CRX(2,3), CRX(4,5), CRX(1,2), CRX(3,4)\n"
