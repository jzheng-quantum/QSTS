...
def _cbref_block(params, wires):
    ...
    p = 12
    for _ in range(6):
        qml.CRX(params[p],     wires=[wires[5], wires[0]])
        qml.CRX(params[p + 1], wires=[wires[4], wires[5]])
        qml.CRX(params[p + 2], wires=[wires[3], wires[4]])
        qml.CRX(params[p + 3], wires=[wires[2], wires[3]])
        qml.CRX(params[p + 4], wires=[wires[1], wires[2]])
        qml.CRX(params[p + 5], wires=[wires[0], wires[1]])
        p += 6


    for q in range(6):
        qml.RX(params[48 + q], wires=wires[q])
    for q in range(6):
        qml.RZ(params[54 + q], wires=wires[q])


def qsts_circuit(params, wires=None):
    
    _cbref_block(params[0:60],    wires=[wires[i] for i in range(6)])
    _cbref_block(params[60:120],  wires=[wires[i] for i in range(6, 12)])
...
class QuantumNeuralNetwork(nn.Module):
    def __init__(
        ...
        self.params = nn.Parameter(
            torch.rand(120, generator=quantum_rng, dtype=torch.float64) * 2 * math.pi
        )
...
def main() -> None:
    ...
    base_name = (
        f"CBref_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ansatz          : CB-ref  (Reference Circuit-Block, RX+RZ single-qubit, CRX entanglers)\n"
        f"  - single-qubit    : RX(θ) RZ(θ) per qubit (12 params per single-qubit layer)\n"
        f"  - entangling      : 6 CRX gates per unit, ring pattern (descending control), repeated x6 (36 CRX/block)\n"
        f"                       per unit: CRX(5,0), CRX(4,5), CRX(3,4), CRX(2,3), CRX(1,2), CRX(0,1)\n"
        f"  - quantum params  : 120 total (Block A: 12+36+12=60; Block B: 12+36+12=60; independent A/B)\n"
