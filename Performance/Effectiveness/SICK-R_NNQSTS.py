...
def _nnqsts_block(params, wires):
...
def qsts_circuit(params, wires=None):
    ...
    _nnqsts_block(params[0:36],  wires=[wires[i] for i in range(6)])
    _nnqsts_block(params[36:72], wires=[wires[i] for i in range(6, 12)])
...
def main() -> None:
    ...
    base_name = (
        f"NNQSTS_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ansatz          : NN-QSTS  (Brick-wall topology, RZ-RY-RZ single-qubit, parameter-free CNOT entanglers)\n"
        f"  - single-qubit    : RZ(θ) RY(θ) RZ(θ) per qubit (Z-Y-Z Euler; 18 params per single-qubit layer)\n"
        f"  - entangling      : 5 CNOT gates per unit (parameter-free), brick-wall pattern, repeated x6 (30 CNOTs/block)\n"
        f"                       per unit: CNOT(0,1), CNOT(2,3), CNOT(4,5), CNOT(1,2), CNOT(3,4)\n"
