...
def _a2aqsts_block(params, wires):
    

    for q in range(6):
        qml.RZ(params[q], wires=wires[q])
    for q in range(6):
        qml.RY(params[6 + q], wires=wires[q])
    for q in range(6):
        qml.RZ(params[12 + q], wires=wires[q])


    qml.CNOT(wires=[wires[5], wires[4]])
    qml.CNOT(wires=[wires[5], wires[3]])
    qml.CNOT(wires=[wires[5], wires[2]])
    qml.CNOT(wires=[wires[5], wires[1]])
    qml.CNOT(wires=[wires[5], wires[0]])

    qml.CNOT(wires=[wires[4], wires[5]])
    qml.CNOT(wires=[wires[4], wires[3]])
    qml.CNOT(wires=[wires[4], wires[2]])
    qml.CNOT(wires=[wires[4], wires[1]])
    qml.CNOT(wires=[wires[4], wires[0]])

    qml.CNOT(wires=[wires[3], wires[5]])
    ...
    qml.CNOT(wires=[wires[2], wires[4]])
    ...
    qml.CNOT(wires=[wires[0], wires[5]])


    for q in range(6):
        qml.RZ(params[18 + q], wires=wires[q])
    for q in range(6):
        qml.RY(params[24 + q], wires=wires[q])
    for q in range(6):
        qml.RZ(params[30 + q], wires=wires[q])


def qsts_circuit(params, wires=None):
    
    _a2aqsts_block(params[0:36],  wires=[wires[i] for i in range(6)])
    _a2aqsts_block(params[36:72], wires=[wires[i] for i in range(6, 12)])
...
def main() -> None:
    ...
    base_name = (
        f"A2AQSTS_{dataset_name}_{topology_name}_{optimizer_name}_"
    ...
    header = (
        ...
        f"  - ansatz          : A2A-QSTS  (All-to-All topology, RZ-RY-RZ single-qubit, parameter-free CNOT entanglers)\n"
        f"  - single-qubit    : RZ(θ) RY(θ) RZ(θ) per qubit (Z-Y-Z Euler; 18 params per single-qubit layer)\n"
        f"  - entangling      : 30 CNOT gates (parameter-free, all ordered pairs ctrl→tgt), NOT repeated\n"
        f"                       order: ctrl=5,4,3,2,1,0 (descending); each controls all other 5 qubits\n"
        f"  - quantum params  : 72 total (Block A: 18+0+18=36; Block B: 18+0+18=36; independent A/B)\n"
